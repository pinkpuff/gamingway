...
class Monster:
 ...
 # This gives the monster one or more additional attack attributes. You can specify
 # it as an index, as a string matching one of the element or status names in the
 # config, or as a list of either of the above. This also automatically sets the
 # monster's "has_attributes" flag.
 def add_attribute(self, attribute):

  # Convert the parameter to a list of attribute flag indexes.
  attributelist = self.config.index_list(attribute, "attributes")
  
  # Then process the list.
  if len(attributelist) > 0:
   for index in attributelist:
    self.attributes.flags[index] = True

   # Make sure we set the "has_attributes" flag.
   self.has_attributes = True

 # This sets the monster's attack attributes to the given list. All flags included
 # in the list will be set in the monster's attributes, and all flags not included
 # will be unset. The flags can be specified by name as per the config or by index.
 # A single string or integer will be interpreted as a list containing only that
 # flag. If this results in the monster having no attributes, its "has_attributes"
 # flag will be unset.
 def set_attribute(self, attribute):
  ...
  # Then process the list.
  for index in range(len(self.config.attribute_names)):
   if index in attributelist:
    self.attributes.flags[index] = True
   else:
    self.attributes.flags[index] = False

  # Update the "has_attributes" flag appropriately.
  self.has_attributes = (len(self.attributes.flags) > 0)

 # This removes an element from the monster's attack attributes. You can specify it
 # as an index, as a string matching one of the element names in the config, or as a
 # list of either of the above. If this results in the monster having no remaining
 # attributes, it will unset the monster's "has_attributes" flag.
 def remove_attribute(self, attribute):

  # Convert the parameter to a list of element flag indexes.
  attributelist = self.config.index_list(attribute, "elements")
 ...
 # This removes an element from the monster's resistances. You can specify it as an
